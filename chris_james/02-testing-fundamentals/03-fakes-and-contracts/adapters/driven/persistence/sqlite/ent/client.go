...
import (
	...
	"github.com/DarrelASandbox/playground-go/chris_james/02-testing-fundamentals/fakes-and-contracts/adapters/driven/persistence/sqlite/ent/migrate"
...
// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *PantryClient) MapCreateBulk(slice any, setFunc func(*PantryCreate, int)) *PantryCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &PantryCreateBulk{err: fmt.Errorf("calling to PantryClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*PantryCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
